...
Since the [stability_analysis](../../Source/ztoolacdc/stability.py#L73) function is called, the results also include detailed system stability properties, such as the application of the Nyquist criterion to determine system stability, the eigenvalue decomposition of the closed-loop impedance matrix to reveal the system oscillatory modes and participating buses for the dominant one, as well as the computation of the passivity index of the different system matrices. The results are displayed during the execution of the script when `verbose=True`. Furthermore, the figures and corresponding text files can be found in the `results_folder` if `make_plot=True` and `save_results=True`, respectively. If only the stability of the system is of interest the calculation of the other metrics can be disabled by setting their corresponding argument to `False`, e.g. when `run_passivity=False`, ` run_nyquist_det=False` the function does not calculate the passivity index and it does not calculate the determinant-based Nyquist criterion, respectively. As seen below, the first case shows no encirclements of the critical point while the second case underneath shows an encirclement as well as a dominant oscillation mode around 60 Hz. 

![Small-signal analysis case 1](../../Doc/EH_stable.png)

![Small-signal analysis case 2](../../Doc/EH_unstable.png)

The instability in the second case can be easily verified by a single EMT simulation as shown in [this work](https://lirias.kuleuven.be/4201452&lang=en); the oscillation amplitudes also validate the insights from the participation factor analysis, i.e. the interaction is dominated by the components at _g2_, i.e. the IBR equivalent, and the AC-side of _MMC2_. Note that due to the nature of EMT simulations, slight oscillatory frequency differences can be occur depending on the simulation time step.
